...
class Reservoir2MLP(nn.Module):
    def __init__(self, reservoir_model, mlp_layers, d_output):
        ...
        for i in range(mlp_layers - 1):
            layers.append(nn.Conv1d(in_channels=d_state, out_channels=d_state, kernel_size=1))
            layers.append(nn.Tanh())

        layers.append(nn.Conv1d(in_channels=d_state, out_channels=d_output, kernel_size=1))
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            x = self.reservoir_model(x)  # (B, d_input, L) - > (B, d_reservoir, 1)

        for layer in self.mlp_layers:  # (B, d_reservoir, 1) - > (B, d_output, 1)
            x = layer(x)
